...
const int N = 1e4 + 5;
string s1;
int ra[N], tempra[N], suf[N], tempsuf[N], n, cnt[N];
int lps[N];

void counting_sort(int k)
{
  int sum = 0, mx = max(256, n);
  ...
  for (int i = 0; i < n; i++)
    tempsuf[cnt[suf[i] + k < n ? ra[suf[i] + k] : 0]++] = suf[i];
  for (int i = 0; i < n; i++)
    suf[i] = tempsuf[i];
}
void LPS()
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    int j = suf[cnt[i] + 1];
    while (match + j < n and match + i < n and s1[match + i] == s1[match + j])
      match++;
    lps[cnt[i]] = match;
    if (match)
      match--;
...
int main()
{
  ...
  for (int test = 1; test <= t; test++)
  {
    ...
    cin >> s1 >> lb >> rb;
    n = s1.size();
    for (int i = 0; i < n; i++) suf[i] = i, ra[i] = s1[i];
    for (int k = 1; k < n; k <<= 1)
    {
      counting_sort(k);
      counting_sort(0);
      ...
      currank = 0;
      for (int i = 1; i < n; i++)
        tempra[suf[i]] = (ra[suf[i]] == ra[suf[i - 1]] and ra[suf[i] + k] == ra[suf[i - 1] + k]) ? currank : ++currank;
      for (int i = 0; i < n; i++)ra[i] = tempra[i];
      //if (ra[suf[n - 1]] == n - 1) break;
    }
    LPS();
    int ans = 0;
    for (int i = 0; i < n; i++)
    {
      int val = min(n - suf[i], rb) - max(lb - 1, lps[i]);
      if (val > 0) ans += val;
    }
